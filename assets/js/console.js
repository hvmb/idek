document.addEventListener("DOMContentLoaded", function() {
        ...
        if (consoleHasFocus) {
            ...
            if (key == "Enter" && consoleReady) {
                ...
                executeCMD(cmdInput).then(result => {
					consoleData.push("cutie@sillydstat:~$ " + cmdInput);
                    result.forEach(line => {
    ...
    writeConsole = function() {
		...
		if (consoleData.length > 11) {
			...
			outputData.push(`cutie@sillydstat:~$ ${cmdInput}${cursor}`);
		} else {
        ...
        return new Promise((resolve, reject) => {
            switch(cmd.toLowerCase()) {
                case "":
                    ...
                    break;
                case "help":
					resolve(["-> help: well u js used so obv yk what this does", "-> stats: stats for nerds- i mean sillydstat.us", "-> rank: biggest attacks from sniggas who hit sillydstat.us", "-> clear: tf do u think?"]);
                    break;
                case "nload":
                    nload();
                    reject();
                ...
                case "login":
                    ...
                    reject();
                case "astrid":
                    resolve(["I'm her #1 Fangirl she the cutest ong"]);
                    break;
                case "qu":
                    resolve(["second cutest, Astrid #1"]);
                    break;
                case "rank":
                    ...
                    break;
                case "rank bypassed":
                    resolve(["Reporting the biggest bypassing attacks bxv has mitigated", "idfk and idfc"])
                    break;
                case "rank raw":
                    resolve(["Reporting the biggest raw attacks bxv has mitigated", "idfk and idfc"])
                    break;
                case "clear":
					consoleData = [];
                    reject();
                    break;
                default: 
                    resolve([`Hey retard! Yea you, ${cmd} isn't a FUCKING command, do you not know how to read? Anyways type help cuz ur so sped, and no it wont get you any mental help `]);
                    break;
            }
        });
    };
